...
A problem we'll face right away when trying to "animate" on the terminal is that we'll need to draw a different 
character to the same part of the terminal. So far, we only know how to write characters or lines to stdout 
...
Thankfully, there are multiple ways to accomplish this, and we'll look at a very simple and limited approach.
...
const repeat = 15;
const time = 200;
const spinnerItems = ['\r|   ', '\r/   ', '\r-   ', '\r\\   '];
let index = -1;
const intervalId = setInterval(() => {
  index++;
  if (index === repeat * spinnerItems.length - 1) {
    clearInterval(intervalId);
  }
  process.stdout.write(spinnerItems[index % spinnerItems.length]);
}, time);
